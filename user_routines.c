...
void Update_OI( void )
{
	...
	if (count >= 15)					// Every 15 loops update state of lights
	{
		...
		if (T_Packet_Data.confidence < CONFIDENCE_THRESHOLD_DEFAULT)			// If were not locked onto the target..
		{
	...
	if (OP_SENSOR_3)					// If the collector has been full for more then 15 loops, its full...
